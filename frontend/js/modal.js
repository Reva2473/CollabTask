...
function renderRoles(roles) {
    ...
    list.innerHTML = roles.map(r => {
        ...
        const memberListHtml = roleMembers.length > 0 
           ? `<div class="mt-3 bg-dark-panel p-2.5 rounded-xl border border-dark-border/50">
                <div class="text-[9px] text-dark-muted mb-2 font-bold uppercase tracking-[0.15em] flex items-center gap-1.5">
                ...
                <div class="flex flex-wrap gap-1.5">
                    ${roleMembers.map(m => `
                        <div class="flex items-center gap-1.5 bg-dark-surface border border-dark-hover px-2 py-1 rounded-lg">
        ...
        return `
        <div class="bg-dark-surface border border-dark-border p-4 rounded-xl flex flex-col gap-3 hover:border-dark-hover transition-all group">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    <div class="w-3.5 h-3.5 rounded-full border border-white/10 shadow-sm" style="background-color: ${r.color}"></div>
                    <span class="text-xs font-bold text-white uppercase tracking-tight">${r.name}</span>
                </div>
                <button onclick="deleteCustomRole('${r.id}')" class="text-dark-muted group-hover:text-red-400 hover:bg-red-500/10 p-1.5 rounded-lg transition-all" title="Delete Role">
            ...
            <div class="pt-2.5 border-t border-dark-border/50">
                <div class="text-[10px] text-dark-muted mb-1.5 font-bold uppercase tracking-wider">Root Access</div>
                <div class="flex items-center gap-2 bg-dark-panel p-2.5 rounded-lg border border-dark-border group-hover:border-dark-hover transition-colors overflow-hidden">
                    <svg class="text-brand-light shrink-0" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
...
window.switchTab = function(tabName) {
    ...
    if(tabName === 'workflow') {
        workflowBtn.className = "text-sm font-semibold border-b-2 border-brand-default text-brand-light px-4 py-2.5 transition-all";
        manageBtn.className = "text-sm font-medium border-b-2 border-transparent text-dark-muted hover:text-white px-4 py-2.5 transition-all";
        paneWorkflow.classList.remove('hidden-pane');
        paneManage.classList.add('hidden-pane');
    } else {
        workflowBtn.className = "text-sm font-medium border-b-2 border-transparent text-dark-muted hover:text-white px-4 py-2.5 transition-all";
        manageBtn.className = "text-sm font-semibold border-b-2 border-brand-default text-brand-light px-4 py-2.5 transition-all";
